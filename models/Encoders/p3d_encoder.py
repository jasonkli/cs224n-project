...
class P3DEncoder(nn.Module):
	...
	def __init__(self, feature_size=2048, attention_size=1024, hidden_decoder_size=512, dropout_rate=0.0):
		
		super().__init__()
		self.feature_transform = nn.Linear(feature_size, attention_size)
		self.hidden_transform = nn.LSTMHighway(feature_size, hidden_decoder_size)
		self.cell_transform = nn.LSTMHighway(feature_size, hidden_decoder_size)
		self.dropout(dropout_rate)


	def forward(self, features_padded, features_actual_lengths):
		sum_features = torch.sum(features_padded, dim=0)
		mean_features = sum_features / torch.unsqueeze(torch.tensor(features_actual_lengths, device=device), dim=1)
		...
		return mapped_features, (hidden_layer1, cell_layer1), (hidden_layer2, cell_layer2)
